...
const getSelectedVeggies = () => {
  ...
  for(let j = 0; j < veggieCheckboxes.length; j++) {
      ...
      for(let k = 0; k < veggieOptions.length; k++) {
          ...
          if(veggieCheckboxes[j].checked && veggieCheckboxes [j].id === veggieOptions [k].id) {
              ...
              selectedVeggie.push(veggieOptions[k]);
...
const makeOptions = () => {
  ...
  for (let i = 0; i < veggieOptions.length; i++) {
    domString += `<div class="form-check">
        <input class="form-check-input veggies" type="checkbox" value="" id="${veggieOptions[i].id}">
...
export default { makeOptions, getSelectedVeggies };
